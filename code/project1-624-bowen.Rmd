...
```
Again, we see a p-value less than 0.05, so we can reject the null hypothesis and assume our time series is stationary. This means we again **won't** have to apply differencing to our data. In this case as well, the variance of our seasonal fluctuations also does not change with time, so we won't need to transform the data (via Box-Cox or other methods).


Let's also take a look at the autocorrelation plots
```{r}
usage %>% ACF(KWH_transformed) %>% autoplot()
```
...
usage_fit <- usage %>%
  model(
    ...
    `Seasonal naïve` = SNAIVE(KWH_transformed)
  )
...
We could also apply an ARIMA model here. In this case, we'll want a seasonal ARIMA model
```{r}
usage_arima <- usage %>% model(ARIMA(KWH_transformed))

usage_fc <- usage_arima %>% forecast(h=12)

usage_fc %>% autoplot(usage)
```

Lastly, we'll write our forecasted values (including April 201)
