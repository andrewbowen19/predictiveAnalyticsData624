...
adf.test(atm$Cash)
```
In this case, we see a p-value of 0.01, so we can reject the null hypothesis and assume our time series is stationary.
...
Our `mable` for the ATM dataset contains an ARIMA model per panel (in this case, per ATM). Each ARIMA model contains different values for $p, q, d$ as parameters. 
